...
# === 版本标记 ===
# 基于 graphify v0.9.5
# 依赖 CLI: graphify-mcp --transport http --port 8765（serve 入口是 pyproject [project.scripts]
...
nohup graphify-mcp --transport http --port 8765 > /tmp/graphify-serve.log 2>&1 &
disown
