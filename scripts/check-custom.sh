...
echo ""
echo "=== 新增文件存在性检查 ==="
# 分层融合自定义面（Task 13-16，feat/codegraph-merge）：scripts/ 下自定义脚本与上游 graphify/ 零交叠
# split_semantic_seed.py=旧图 semantic 种子拆分；rebuild_entry.py=单一重建入口（Task 09 换源：
# extract→build→to_json→rebuild_fts→分析，codegraph sync/adapter 退役）；run_analysis.py=analysis-only 编排器
# Task 09：adapter.py 已整体退役（删）；_symbol_short_name 迁至 scripts/symbol_utils.py（structure_queries 消费）
...
# native-indexing Task 03（docstring 字段提取 Python/JS/TS）：fixture 驱动测试与三个 fixture——上游 merge 丢弃时守护必须告警
# native-indexing Task 05（FTS 缓存模块）：scripts/fts_cache.py 缓存构建模块 + tests/test_fts_cache.py Seam 2 产物对测试——上游 merge 丢弃时守护必须告警
# native-indexing Task 06（点查类工具换源）：tests/test_get_node_fts.py Seam 3 工具/查询面测试（get_node 四档走 FTS 缓存）——上游 merge 丢弃时守护必须告警
# native-indexing Task 07（ranked 换源 + failed_refs 持久化）：tests/test_failed_refs_persistence.py Seam 2 产物对测试（build/to_json/watch 的 failed_refs 跨边界存活）——上游 merge 丢弃时守护必须告警
# native-indexing Task 09（scripts 编排重组 + 一次性换源）：symbol_utils.py（adapter 退役后短名事实源）+ mini-graph.json（旧 codegraph DB fixture 替换）+ 本票重写核心测试（rebuild_entry/run_analysis/graph_diff_sync）——上游 merge 丢弃时守护必须告警
# native-indexing Task 10（serve 内置 watcher）：graphify/serve_watcher.py 纯自定义新增 + tests/test_serve_watcher.py Seam 4 触发链端到端测试——上游 merge 丢弃时守护必须告警
# per-project-watcher Task 01（锁原语提升）：graphify/rebuild_lock.py 跨进程互斥 mkdir 原子锁（自 scripts/rebuild_entry.py 逐字提升，单一事实源）+ tests/test_rebuild_lock.py 模块级直测——上游 merge 丢弃时守护必须告警
