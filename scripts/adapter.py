...
# === B3 合并图短名（Phase 4 Task 9）================================================
# Task 08：dispatch 概念退役——_dispatch_candidate/_edge_dispatch_info 双副本已删（语义由
# 边属性 confidence/resolved_by 原生承载，见 graphify/serve.py）。_symbol_short_name 保留：
# serve 持自包含副本（graphify 包不反向依赖 scripts/；rebuild_entry._parse_refresh 先例，
# 防漂移锁定靠 tests/test_dispatch_trace.py 一致性测试），scripts/structure_queries.py 也消费它。
