...
# ── Task 03: docstring field (native-indexing six-piece contract) ─────────────
# The ``docstring`` top-level field on symbol nodes: raw text, 1500-char cap
# with a semantic-break fallback in the [1350, 1500] window, a quantized
# `` …[+N chars truncated]`` marker, and a <5-char noise filter (ticket
# 03-field-contract, sections B1-B3). Python is backfilled at the existing
# rationale stopping point (the rationale node/edge behavior itself is
# untouched); JS/TS JSDoc blocks use a new declaration-preceding-comment pass.
# The rationale label path (_shorten_rationale_label) is deliberately separate:
# that is a collapsed 80-char graph label, this is the verbatim field.

_DOCSTRING_LIMIT = 1500
_DOCSTRING_WINDOW = 150  # semantic-break search window [limit-window, limit)
_DOCSTRING_TRUNCATED_MARKER = " …[+{n} chars truncated]"
...
def _truncate_docstring(text: str) -> str:
    ...
    if len(text) <= _DOCSTRING_LIMIT:
        return text
    cut = None
    for i in range(_DOCSTRING_LIMIT - 1, _DOCSTRING_LIMIT - _DOCSTRING_WINDOW - 1, -1):
        ch = text[i]
        if ch == "\n":
            # Paragraph boundary (\n\n, or the \n of a \r\n\r\n pair).
            if text[i - 1:i + 1] == "\n\n" or text[i - 3:i + 1] == "\r\n\r\n":
                cut = i + 1
                break
            cut = i + 1  # line boundary
            break
        if ch in "。.!?" and (i + 1 >= len(text) or text[i + 1].isspace()):
            cut = i + 1  # sentence boundary: terminator + whitespace
            break
    if cut is None:
        cut = _DOCSTRING_LIMIT
    return text[:cut] + _DOCSTRING_TRUNCATED_MARKER.format(n=len(text) - cut)


_PY_DOCSTRING_QUOTES = ('"""', "'''", '"', "'")


def _python_first_statement_docstring(body_node, source: bytes) -> str | None:
    """Contract B1: the docstring is the body's first child statement, an
    ``expression_statement`` containing exactly one plain ``string`` node — no
    concatenation, no f/b/r prefix. Returns the quote-stripped, stripped text,
    or None when the first-statement rule fails or it strips to < 5 chars
    ...
    if len(named) != 1 or named[0].type != "string":
        return None
    raw = source[named[0].start_byte:named[0].end_byte].decode("utf-8", errors="replace")
    for q in _PY_DOCSTRING_QUOTES:
        if raw.startswith(q):
            content = raw[len(q):]
    ...
    return None  # f/b/r-prefixed or otherwise not a plain quoted string
...
def _extract_python_rationale(path: Path, result: dict) -> None:
    ...
    def _backfill_docstring(nid: str, body_node) -> None:
        """Task 03: attach the precise first-statement docstring to the symbol
        node as a top-level field. Pure field addition — the rationale node and
        edge behavior above is untouched (that path keeps its own loose
        ``_get_docstring``). Local/nested functions that never became nodes are
        no-ops (``nid`` absent from ``nodes_by_id``)."""
        if nid not in nodes_by_id:
            return
        text = _python_first_statement_docstring(body_node, source)
        if text is None:
            return
        nodes_by_id[nid]["docstring"] = _truncate_docstring(text)
    ...
    if not _is_autogenerated_python(source):
        ...
        _backfill_docstring(file_nid, root)
...
def extract_js(path: Path) -> dict:
    ...
    if "error" not in result:
        ...
        _extract_js_docstrings(path, result)
...
# ── JS/TS docstring field: declaration-preceding JSDoc association (Task 03) ──
# New pass (unlike Python's backfill at the existing rationale stopping point):
# a JSDoc ``/** … */`` block comment is associated with the function/method/
# class declaration it immediately precedes, and its prose is stored as the
# symbol node's ``docstring`` top-level field. Mirrors the generic extractor's
# node-id construction so the field lands on the real node. The rationale pass
# above is untouched.

# `export function/class/const` wraps the declaration; unwrap to reach it.
_JS_DOCSTRING_EXPORT_TYPES = frozenset({
...
def _strip_jsdoc_block(raw: str) -> str:
    """Strip a JSDoc ``/** … */`` block down to its prose: drop the opening
    ``/**`` / closing ``*/`` and each line's leading ``*`` continuation marker."""
    text = raw.strip()
    if text.startswith("/**"):
        text = text[3:]
    if text.endswith("*/"):
...
def _extract_js_docstrings(path: Path, result: dict) -> None:
    """Task 03: associate JSDoc block comments with the function/method/class
    they immediately precede, storing the prose as a top-level ``docstring``
    field on the symbol node. Pure field addition; the JS/TS rationale and
    doc-reference post-pass above is untouched."""
    try:
        ...
        if suffix in (".ts", ".tsx", ".mts", ".cts"):
            lang_fn = getattr(tsts, "language_tsx" if suffix == ".tsx" else "language_typescript")
        else:
            lang_fn = tsjs.language
    ...
    def walk(node) -> None:
        if node.type == "comment":
            ...
            if raw.lstrip().startswith("/**"):
                ...
                if nxt is not None:
                    nid = _js_docstring_target_nid(nxt, source, stem)
                    if nid in nodes_by_id:
                        doc = _strip_jsdoc_block(raw).strip()
                        if len(doc) >= 5:
                            nodes_by_id[nid]["docstring"] = _truncate_docstring(doc)
        for child in node.children:
            walk(child)
