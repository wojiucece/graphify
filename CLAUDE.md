...
- **上游行为变更会撞 fork 契约测试**（0.9.65）：上游 #3405 让**嵌套函数提取为节点**（`engine._scan_python_nested_function_declarations`），与 fork 独有的 `tests/test_extraction_contract.py` 三个断言冲突（该文件 164 行 + fixture 31 行**均为 fork 独有**，上游零改动 → 责任在"上游改行为"）。处置：标 `xfail(strict=False)` 待决策，不改生产代码。**同类风险持续存在**，每次升级后都应做上一条的失败集合 diff
