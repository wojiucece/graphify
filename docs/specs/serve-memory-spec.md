...
- **双写路径（grill Q2 修正，正确性必要）**：rebuild_entry 重建完成 + `serve_watcher._run_pipeline` 末尾**各记一次** `source_count`（I3-勘误-2 后同记 `source_max_mtime`，门控 stat-to-stat 参照）——只写 hook 面的话 watcher 重建后 count 停留旧值 → 门控误判陈旧 → 冗余重建。计数统一调 **`extract.collect_files`**（extract.py:8126，发现规则本体，零新面，依赖方向合法）。**锁 owner 约束（实施红线）**：`_run_pipeline`（serve_watcher.py:740-808）当前不写状态文件（状态文件是 rebuild_entry 职责，schema v2）——新增双写调用 `rebuild_entry._write_state`（rebuild_entry.py:44）时必须仍持 rebuild_lock：该函数读锁 pid 判断 owner（:433"锁没了则拒写"），锁外调用会**静默丢 count**；实施时确认 `_flush_batch` 的锁生命周期（acquire → finally 释放）覆盖到 pipeline 末尾
